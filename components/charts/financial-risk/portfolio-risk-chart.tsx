...
export function PortfolioRiskChart({ data, metadata, sizeType }: PortfolioRiskChartProps) {
  ...
  const renderChart = (width: number, height: number) => {
    ...
    // Draw bars with risk-specific styling
    labels.forEach((label: string, index: number) => {
      const barHeight = (values[index] / maxValue) * chartHeight;
      const x = padding + index * (barWidth + barSpacing) + barSpacing / 2;
      const y = height - padding - barHeight;

      // Draw bar with risk color coding
      let barColor = colors[index] || '#10B981';
      if (values[index] > 15.0) barColor = '#EF4444'; // High risk
      else if (values[index] <= 5.0) barColor = '#10B981'; // Low risk
      else barColor = '#F59E0B'; // Moderate risk
    ...
    if (sizeType !== 'small') {
      ...
      if (sizeType === 'large') {
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Asset Class', width / 2, height - 5);
      }
    }
  };
