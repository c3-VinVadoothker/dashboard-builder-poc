...
export function VolatilityAnalysisChart({ data, metadata, sizeType }: VolatilityAnalysisChartProps) {
  ...
  useEffect(() => {
    ...
    // Draw bars with volatility-specific styling
    labels.forEach((label: string, index: number) => {
      const barHeight = (values[index] / maxValue) * chartHeight;
      const x = padding + index * (barWidth + barSpacing) + barSpacing / 2;
      const y = height - padding - barHeight;

      // Draw bar with volatility color coding
      let barColor = colors[index] || '#10B981';
      if (values[index] > 20.0) barColor = '#EF4444'; // High volatility
      else if (values[index] <= 10.0) barColor = '#10B981'; // Low volatility
      else barColor = '#F59E0B'; // Moderate volatility
    ...
    if (sizeType !== 'small') {
      ...
      if (sizeType === 'large') {
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Investment Category', width / 2, height - 5);
      }
    }
