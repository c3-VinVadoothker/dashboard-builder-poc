...
interface ReadmissionRatesChartProps {
  data: any;
  metadata?: any;
  sizeType: 'small' | 'medium' | 'large';
}

export function ReadmissionRatesChart({ data, metadata, sizeType }: ReadmissionRatesChartProps) {
  ...
  useEffect(() => {
    ...
    // Draw bars with readmission-specific styling
    labels.forEach((label: string, index: number) => {
      const barHeight = (values[index] / maxValue) * chartHeight;
      const x = padding + index * (barWidth + barSpacing) + barSpacing / 2;
      const y = height - padding - barHeight;

      // Draw bar with readmission color coding (lower is better)
      let barColor = colors[index] || '#10B981';
      if (values[index] > 12.0) barColor = '#EF4444'; // High readmission
      else if (values[index] <= 5.0) barColor = '#10B981'; // Low readmission
      else barColor = '#F59E0B'; // Moderate readmission
    ...
    if (sizeType !== 'small') {
      ...
      if (sizeType === 'large') {
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Readmission Rate (%)', 0, 0);
        ctx.restore();
