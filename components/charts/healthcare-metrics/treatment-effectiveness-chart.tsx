...
export function TreatmentEffectivenessChart({ data, metadata, sizeType }: TreatmentEffectivenessChartProps) {
  ...
  useEffect(() => {
    ...
    const maxValue = 5.0; // Effectiveness is out of 5
    ...
    // Draw bars with treatment effectiveness styling
    labels.forEach((label: string, index: number) => {
      const barHeight = (values[index] / maxValue) * chartHeight;
      const x = padding + index * (barWidth + barSpacing) + barSpacing / 2;
      const y = height - padding - barHeight;

      // Draw bar with effectiveness color coding
      let barColor = colors[index] || '#10B981';
      if (values[index] < 4.0) barColor = '#EF4444'; // Below target
      else if (values[index] >= 4.5) barColor = '#10B981'; // Excellent
    ...
    if (sizeType !== 'small') {
      ...
      if (sizeType === 'large') {
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Effectiveness Score', 0, 0);
        ctx.restore();
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Specialty', width / 2, height - 5);
      }
    }
