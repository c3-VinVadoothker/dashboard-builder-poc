...
interface PRDAnalysisChartProps {
  data: any;
  metadata?: any;
  sizeType: 'small' | 'medium' | 'large';
}

export function PRDAnalysisChart({ data, metadata, sizeType }: PRDAnalysisChartProps) {
  ...
  useEffect(() => {
    ...
    // Draw bars with PRD-specific styling
    labels.forEach((label: string, index: number) => {
      const barHeight = (values[index] / maxValue) * chartHeight;
      const x = padding + index * (barWidth + barSpacing) + barSpacing / 2;
      const y = height - padding - barHeight;

      // Draw bar with PRD-specific color coding
      let barColor = colors[index] || '#10B981';
      if (values[index] < 0.8) barColor = '#EF4444'; // Below standard
      else if (values[index] > 1.0) barColor = '#F59E0B'; // Above standard
      else barColor = '#10B981'; // Within standard
    ...
    // Draw PRD-specific reference lines (ideal range)
