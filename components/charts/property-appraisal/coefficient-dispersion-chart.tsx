...
interface CoefficientDispersionChartProps {
  data: any;
  metadata?: any;
  sizeType: 'small' | 'medium' | 'large';
}

export function CoefficientDispersionChart({ data, metadata, sizeType }: CoefficientDispersionChartProps) {
  ...
  useEffect(() => {
    ...
    // Draw bars with COD-specific styling
    labels.forEach((label: string, index: number) => {
      const barHeight = (values[index] / maxValue) * chartHeight;
      const x = padding + index * (barWidth + barSpacing) + barSpacing / 2;
      const y = height - padding - barHeight;

      // Draw bar with gradient for COD
      const gradient = ctx.createLinearGradient(0, y, 0, y + barHeight);
      gradient.addColorStop(0, colors[index] || '#10B981');
    ...
    if (sizeType !== 'small') {
      ...
      if (sizeType === 'large') {
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Coefficient of Dispersion (%)', 0, 0);
        ctx.restore();
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Property Type', width / 2, height - 5);
      }
    }

    // Draw COD-specific reference lines
    if (metadata && sizeType !== 'small') {
      ...
      if (standardLower !== undefined) {
        ...
        ctx.font = '10px Arial';
        ctx.fillText('Min Standard', padding + 5, y - 5);
      }
      ...
      if (standardUpper !== undefined) {
        ...
        ctx.font = '10px Arial';
        ctx.fillText('Max Standard', padding + 5, y - 5);
      }
    }
