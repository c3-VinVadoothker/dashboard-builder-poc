...
export function CustomerSatisfactionChart({ data, metadata, sizeType }: CustomerSatisfactionChartProps) {
  ...
  useEffect(() => {
    ...
    const maxValue = 5.0; // Satisfaction is out of 5
    ...
    // Draw bars with satisfaction-specific styling
    labels.forEach((label: string, index: number) => {
      const barHeight = (values[index] / maxValue) * chartHeight;
      const x = padding + index * (barWidth + barSpacing) + barSpacing / 2;
      const y = height - padding - barHeight;

      // Draw bar with satisfaction color coding
    ...
    if (sizeType !== 'small') {
      ...
      if (sizeType === 'large') {
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Service Category', width / 2, height - 5);
      }
    }
