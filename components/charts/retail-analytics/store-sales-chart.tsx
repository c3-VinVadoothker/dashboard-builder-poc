...
export function StoreSalesChart({ data, metadata, sizeType }: StoreSalesChartProps) {
  ...
  useEffect(() => {
    ...
    // Draw bars with retail-specific styling
    labels.forEach((label: string, index: number) => {
      const barHeight = (values[index] / maxValue) * chartHeight;
      const x = padding + index * (barWidth + barSpacing) + barSpacing / 2;
      const y = height - padding - barHeight;

      // Draw bar with retail color coding
      let barColor = colors[index] || '#10B981';
      if (values[index] < 80000) barColor = '#EF4444'; // Below target
      else if (values[index] > 150000) barColor = '#10B981'; // Above target
      else barColor = '#3B82F6'; // Within target
      ...
      if (sizeType !== 'small') {
        ...
        ctx.fillText(`$${(values[index] / 1000).toFixed(0)}k`, x + barWidth / 2, y - 5);
      }
    ...
    if (sizeType !== 'small') {
      ...
      if (sizeType === 'large') {
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Sales Revenue ($)', 0, 0);
        ctx.restore();
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Store Location', width / 2, height - 5);
      }
    }
