...
export function InventoryLevelsChart({ data, metadata, sizeType }: InventoryLevelsChartProps) {
  ...
  useEffect(() => {
    ...
    if (sizeType !== 'small') {
      ...
      for (let i = 0; i <= tickCount; i++) {
        ...
        ctx.textAlign = 'right';
        ctx.fillText(`${value.toFixed(1)}x`, padding - 8, y + 3);
      }
    }

    // Draw bars with inventory-specific styling
    labels.forEach((label: string, index: number) => {
      const barHeight = (values[index] / maxValue) * chartHeight;
      const x = padding + index * (barWidth + barSpacing) + barSpacing / 2;
      const y = height - padding - barHeight;

      // Draw bar with inventory color coding
      let barColor = colors[index] || '#10B981';
      if (values[index] < 4.0) barColor = '#EF4444'; // Below target
      else if (values[index] > 6.0) barColor = '#F59E0B'; // Above target
      else barColor = '#10B981'; // Within target
      ...
      if (sizeType !== 'small') {
        ...
        ctx.fillText(`${values[index].toFixed(1)}x`, x + barWidth / 2, y - 5);
      }
    ...
    if (sizeType !== 'small') {
      ...
      if (sizeType === 'large') {
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Turnover Rate (x)', 0, 0);
        ctx.restore();
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Product Category', width / 2, height - 5);
      }
    }
